...
getConditionEvents <- function ( condition, observation_period, date_column = !!as.symbol("condition_start_date") ){

  date_sym <- rlang::enexpr( date_column )

  inner_join( condition, observation_period, by = "person_id" ) %>%
    mutate( event_day = as.integer( (!!date_sym) - observation_period_start_date ) ) %>%
    filter( event_day >= 0,
